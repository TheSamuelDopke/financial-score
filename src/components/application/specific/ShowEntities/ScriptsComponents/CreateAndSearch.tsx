...
export const useEntitiesSearchFunc = (searchValues: Partial<TypeSearch>) => {
  ...
  //Usamos useMemo para que apenas uma vez populemos o banco, que é quando não existem dados!
   useMemo(() => {
    seedDatabase().catch((err) => console.error("Erro no seed: ", err));
  }, []);


  const result = useLiveQuery(async (): Promise<Entities[]> => {
    ...
    if (type === "cpf" || type === "cnpj") {
      ...
      return (await db.entities
        .where("type")
        .equals(entityType)
        .filter((item) => item.cpfCnpj.startsWith(cleanQuery))
        .toArray()) as Entities[];
    }
